...
public class CalloutInvoice extends CalloutEngine
{
	...
	{	
		...
		if (A_Tab == null)
			return "";
		int activeCallout = A_Tab.getActiveCallouts().length;
		if (activeCallout > 1)
			return "";
		boolean IsSOTrx = "Y".equals(Env.getContext(A_Ctx, A_WindowNo, "IsSOTrx"));
		Integer C_BPartner_ID = (Integer)A_Tab.getValue("C_BPartner_ID");
		if (C_BPartner_ID == null || C_BPartner_ID.intValue() == 0)
			return "";
		...
		Integer C_PaymentTerm_ID = (Integer)A_Tab.getValue("C_PaymentTerm_ID");
		if (C_PaymentTerm_ID == null || C_PaymentTerm_ID.intValue() == 0)
		{
			MPaymentTerm pt = null;
		if (IsSOTrx)
			pt = (MPaymentTerm)bp.getC_PaymentTerm();
		else 
			pt = (MPaymentTerm)bp.getPO_PaymentTerm();
		if (pt == null || pt.getC_PaymentTerm_ID()==0)
			return "";
		C_PaymentTerm_ID = pt.getC_PaymentTerm_ID();
		A_Tab.setValue("C_PaymentTerm_ID", C_PaymentTerm_ID);			
		}
		
		Timestamp duedate = MPaymentTerm.dueDate(C_PaymentTerm_ID, datedoc);
		if (duedate != null)
			A_Tab.setValue("DateConfirm", duedate);
			return "";		
	}
	
	public String QuedanPT_BPartner (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{	
		Object A_Value = value;
		Properties A_Ctx = ctx;
		int A_WindowNo = WindowNo;
