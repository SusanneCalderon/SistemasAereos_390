...
public class MOrder extends X_C_Order implements DocAction
{
	...
	public String completeIt()
	{
		...
		setDefiniteDocumentNo();
		//SHW SA
		List<MInvoice> invs = new Query(getCtx(), MInvoice.Table_Name, "c_order_ID=? and docstatus = 'DR'", get_TrxName())
			.setParameters(getC_Order_ID())
			.list();
		for (MInvoice inv:invs)
		{
			if (inv.getLines().length == 0)
			{
				inv.setC_Order_ID(0);
				inv.delete(true);
			}
	...
	private void correctLines(MInvoice invoice)
	{
		if (invoice.getLines().length <=8)
				return; 
		String description = "";
