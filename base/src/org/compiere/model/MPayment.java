...
*/
	...
	public String prepareIt()
	{
		if ( isCashTrx() && !MSysConfig.getBooleanValue("CASH_AS_PAYMENT", true , getAD_Client_ID())) {
			...
			MCash cash = getCash(getC_CashBook_ID(), getDateAcct());
			if (cash == null || cash.get_ID() == 0)
			{
				m_processMsg = "@La caja no está abierta@";
				return DocAction.STATUS_Invalid;
			}
		}
		log.info(toString());
	...
	public String completeIt()
	{
		...
		if ( isCashTrx() && !MSysConfig.getBooleanValue("CASH_AS_PAYMENT", true , getAD_Client_ID())) {
			...
			if (getC_Charge_ID() > 0)
			{
				cl.setC_Charge_ID(getC_Charge_ID());
				cl.setCashType(MCashLine.CASHTYPE_Charge);
			}

			if (getC_BPartner_ID() != 0 && getC_Order_ID() != 0 && MPaymentAllocate.get(this).length == 0)
			{
				cl.setCashType("P");
			}*/
			cl.setDescription("Pago #" + getDocumentNo());
			cl.setC_Currency_ID( this.getC_Currency_ID() );
			cl.setC_Payment_ID( getC_Payment_ID() ); // Set Reference to payment.
			cl.set_Value("C_BPartner_ID", getC_BPartner_ID());
			MPaymentAllocate[] pAllocs = MPaymentAllocate.get(this);
			if (pAllocs.length == 0)
				cl.set_Value(COLUMNNAME_C_Project_ID, getC_Project_ID());
			...
			if (!getC_DocType().getDocBaseType().equals(MDocType.DOCBASETYPE_ARReceipt))
			{
				amt = amt.negate();
			}
			...
			if (getC_DocType().getDocBaseType().equals(MDocType.DOCBASETYPE_ARReceipt))
			{
				if (MPayment.TENDERTYPE_Cash.equals(get_ValueAsString("shw_tenderType_Cash")))
					cl.set_Value(MPayment.COLUMNNAME_TenderType, MPayment.TENDERTYPE_Cash);
				else
				{
					cl.set_Value(MPayment.COLUMNNAME_TenderType, MPayment.TENDERTYPE_Check);
					cl.set_Value(MPayment.COLUMNNAME_CheckNo, getCheckNo());
				}				
			}
			...
			if (get_ValueAsInt(MCashLine.COLUMNNAME_C_CashLine_ID)> 0)
			{
				MCashLine prepay = new MCashLine(getCtx(), get_ValueAsInt(MCashLine.COLUMNNAME_C_CashLine_ID), get_TrxName());
				cl.set_Value("C_PaymentAssigned_ID", prepay.getC_Payment_ID()); //cashline assigned zu sich selbst
				cl.saveEx();
				if (!isReceipt())
				{
					BigDecimal prepayamt = prepay.getAmount();
					BigDecimal diff = prepayamt.add(getPayAmt());
					if (prepay.getC_Cash().getDocStatus().equals(MPayment.DOCSTATUS_Drafted)
							|| prepay.getC_Cash().getDocStatus().equals(MPayment.DOCSTATUS_InProgress))
					{
						prepay.setAmount(diff);
						if (diff.compareTo(Env.ZERO) == 0)
						prepay.set_Value("isAsignedToPrepayCash", true);
						prepay.saveEx();		
					}
					else if (prepay.getC_Cash().getDocStatus().equals("CO"))
					{

						prepay.set_Value("C_PaymentAssigned_ID", getC_Payment_ID());
						prepay.saveEx();
						MCashLine cl_anticipo = new MCashLine(cash);
						MCashLine.copyValues(prepay, cl_anticipo);
						cl_anticipo.setC_Cash_ID(cash.getC_Cash_ID());
						cl_anticipo.setAmount(cl.getAmount().negate());
						cl_anticipo.setC_Payment_ID(prepay.getC_Payment_ID());
						cl_anticipo.set_Value("C_PaymentAssigned_ID", getC_Payment_ID());
						cl_anticipo.setCashType(MCashLine.CASHTYPE_GeneralReceipts);
						//cl_anticipo.setC_Charge_ID(prepay.getC_Payment().getC_Charge_ID());
						cl_anticipo.set_Value("isAsignedToPrepayCash", true);
						cl_anticipo.setDescription("Anticipo asignado");
						cl_anticipo.saveEx();
						MCashLine cl_diff = new MCashLine(cash);
						MCashLine.copyValues(prepay, cl_diff);
						cl_diff.setC_Cash_ID(cash.getC_Cash_ID());
						cl_diff.setAmount(diff.negate());
						cl_diff.setC_Payment_ID(prepay.getC_Payment_ID());
						cl_diff.set_Value("C_PaymentAssigned_ID", getC_Payment_ID());
						cl_diff.setCashType(MCashLine.CASHTYPE_Charge);
						cl_diff.setC_Charge_ID(prepay.getC_Payment().getC_Charge_ID());
						cl_diff.set_Value("isAsignedToPrepayCash", true);
						cl_diff.setDescription("Reembolso de anticipo");
						cl_diff.saveEx();					
					}
				}
			}
	...
	private boolean allocatePaySelection()
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				int C_CashLine_ID = new Query(Env.getCtx(), MCashLine.Table_Name, "c_payment_ID=?", get_TrxName())
				.setParameters(getC_Payment_ID())
				.firstId();
				aLine.setPaymentInfo(getC_Payment_ID(), C_CashLine_ID);
				if (!aLine.save(get_TrxName()))
