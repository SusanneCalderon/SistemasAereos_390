...
public class MRfQResponse extends X_C_RfQResponse
{
		...
		int C_BPartner_ID, int C_BPartner_Location_ID, int AD_User_ID)
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			MRfQLine  rfqLine = lines[i];
			line.set_ValueOfColumn(MWarehouse.COLUMNNAME_M_Warehouse_ID, rfqLine.get_Value(MWarehouse.COLUMNNAME_M_Warehouse_ID));
	...
	public boolean sendRfQ()
	{
		...
		//
		String message = "";
		int R_MailText_ID = getRfQ().get_ValueAsInt("R_MailText_ID");
		if (R_MailText_ID >0)
		{
			MMailText mText = new MMailText(getCtx(), R_MailText_ID, get_TrxName());
			mText.setUser(to);	//	variable context
			mText.setBPartner(getC_BPartner_ID());
			mText.setPO(m_rfq);
			message = mText.getMailText(true);
			MUser salesrep = (MUser)m_rfq.getSalesRep();
			if (mText.getMailText3().contains("Atentamente"))
				message = message + " " + salesrep.getName();
		}
		//String message = m_rfq.getDescription();
		else
		{
			if (m_rfq.getHelp() != null)
				message += "\n" + m_rfq.getHelp();
			if (message.equals(""))
				message = getName();
		}
	...
	private File getRFQAttachment()
	{
		MAttachment att = m_rfq.getAttachment(true);
		if (att==null)
			return null;
		MAttachmentEntry []  entries = att.getEntries();
		MAttachmentEntry entry = entries[0]; 
		return att.getEntryFile(1, entry.getName());
	}
