	...
	protected String doIt() throws Exception
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			else 	//	Invoice Basis
			{
				if (m_com.isListDetails())
				{
					sql.append("SELECT h.C_Currency_ID, multiplier*l.LineNetAmt, l.QtyInvoiced, "
			...
			else 
			{
				...
				else
				{
					log.warning("Not 1 User/Contact for C_BPartner_ID=" 
						+ m_com.getC_BPartner_ID() + " but " + users.length);
					sql.append(" AND h.SalesRep_ID NOT IN (SELECT AD_User_ID FROM AD_User WHERE C_BPartner_ID=")
						.append(m_com.getC_BPartner_ID()).append(")");
				}
