		...
		try
		{
			...
			while (rs.next ())
			{
				...
				if (!oldDocumentNo.equals(cmpDocumentNo))
				{
			        ...
			        params.add(imp.getDocumentNo());
			        BigDecimal c_invoice_count = new Query(imp.getCtx(), MInvoice.Table_Name, "Documentno =?", imp.get_TrxName())
			            .setParameters(params.toArray())
			            .setClient_ID()
			            .aggregate("*", Query.AGGREGATE_COUNT);
			        if (c_invoice_count.longValue()> 0)
			        {
			        	imp.setI_ErrorMsg("Una factura con este No de Documento ya existe");
			        	imp.saveEx();
			        	continue;
			        }
			        //SHW
				...
				if (!oldDocumentNo.equals(cmpDocumentNo))
				{
					if (invoice != null)
					{
						if (!(m_docAction == null || m_docAction.equals("")))
					...
					if (invoice == null)
					{
						...
						if (imp.get_ValueAsInt("User1_ID") > 0)
							invoice.set_ValueOfColumn("User1_ID", imp.get_ValueAsInt("User1_ID"));
						//
						invoice.saveEx();
						noInsert++;
