...
import org.compiere.model.X_I_Payment;import org.compiere.process.ProcessInfoParameter;
...
public class ImportPayment extends SvrProcess
{
	...
	private Boolean 		m_onlyValidate = false; 
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			if (name.equals("AD_Org_ID"))
				...
				m_onlyValidate = "Y".equals(para[i].getParameter());
			else
	...
	protected String doIt() throws Exception
	{
		...
		try
		{
			...
			{ 
				...
				//SHW schon angelegte Zahlungen nicht neu importieren
				BigDecimal count = new Query(getCtx(), MPayment.Table_Name, "documentno =?", get_TrxName())
					.setParameters(cmpDocumentNo)
					.aggregate("*", Query.AGGREGATE_COUNT);
				if (count.longValue() > 0)
				{
					imp.setI_ErrorMsg("Un pago/recuado con este # documento ya eciste");
					imp.saveEx();
					continue;
				}
			...
			//	payment.setC_ConversionType_ID(imp.getC_ConversionType_ID());
				...
				//SHW
				payment.setC_CashBook_ID(imp.get_ValueAsInt("C_CashBook_ID"));
				payment.set_ValueOfColumn("shw_tenderType_Cash", imp.get_Value("shw_tenderType_Cash"));
				payment.set_ValueOfColumn("shw_codTransaccion", imp.get_Value("shw_codTransaccion"));
				if (imp.get_ValueAsInt("User1_ID") > 0)
					payment.set_ValueOfColumn("User1_ID", imp.get_ValueAsInt("User1_ID"));
				//
