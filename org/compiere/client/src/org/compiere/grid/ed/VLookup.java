...
import org.compiere.apps.search.InfoPriceRate;
...
//				}
		...
		else if (col.equals("M_Product_ID"))
		{
			...
			if(m_mField != null)
			{
				...
				MField field = new MField(Env.getCtx(), m_mField.getAD_Field_ID(), null);
				multipleSelection = field.get_ValueAsBoolean("IsMultiselection");
				...
				multipleSelection = (MOrderLine.Table_ID ==  AD_Table_ID) || 
									...
									(MPromotionGroupLine.Table_ID == AD_Table_ID) ||
		...
		else if (col.equals("C_BPartner_ID"))
		{
			...
			if(m_mField != null)
			{
				int AD_Table_ID = MColumn.getTable_ID(Env.getCtx(), m_mField.getAD_Column_ID(), null);
				// TODO hard-coded - add to AD_Column?
				multipleSelection = (MPromotionPreCondition.Table_ID ==  AD_Table_ID) || 
									(MInvoiceBatch.Table_ID == AD_Table_ID ) ||
									(MProductPO.Table_ID == AD_Table_ID||
									1000025 == AD_Table_ID);
			}
		...
		else if (col.equals("C_Invoice_ID"))
		{
			...
			{
				String trxName = Trx.createTrxName();
				MInvoice invoice = new MInvoice(Env.getCtx(), record_id, trxName);
				m_isSOMatch = (m_isSOTrx && invoice.isSOTrx());
				Trx.get(trxName, false).close();
			}
			//
			if(m_mField != null)
		...
		else if (col.equals("LG_Route_ID"))
		{
			resetTabInfo();
			//
			setIsSOTrx(m_isSOTrxEnvOverride, false);
			...
			{
				String trxName = Trx.createTrxName();
				MLGRoute route =  new MLGRoute(Env.getCtx(), record_id, trxName);
				Trx.get(trxName, false).close();
			}
			//
			if(m_mField != null)
		...
		else if (col.equals("LG_ProductPriceRate ID"))
		{
			...
			InfoPriceRate ip = new InfoPriceRate(frame, true, m_lookup.getWindowNo(), record_id,
		...
		else	//	General Info
		{
			...
			if(m_mField != null)
			{
				int AD_Table_ID = MColumn.getTable_ID(Env.getCtx(), m_mField.getAD_Column_ID(), null);
				// TODO hard-coded - add to AD_Column?
				multipleSelection = (1000009 == AD_Table_ID) ||
									(1000008 == AD_Table_ID || AD_Table_ID==749
									);
			}
			Info ig = Info.create (frame, true, m_lookup.getWindowNo(), m_tableName, 
					m_keyColumnName, record_id, queryValue, multipleSelection, true, whereClause);
